...
## [2026-07-23] 修正 trace_check.py 的多段前綴 REQ ID 解析

- `REQ_ROW` 正規表達式原本只認單段前綴（`REQ-UART-001`），遇到多段前綴
  （例：`REQ-I3C-SDR-001`）會直接抓不到、不報錯也不出現在追溯結果裡。
  修正為 `REQ-[A-Z0-9]+(?:-[A-Z0-9]+)*-\d+`，最後一段固定是數字編號，
  前面允許任意段數。已用單段（回歸）與多段（新案例）ID 各自驗證，
  `req_prefix()` 本來就用 `rsplit("-", 1)` 從右邊切，不需要跟著改。
